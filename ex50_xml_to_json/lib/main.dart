...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Ex50 XML To JSON'),
    );
  }
}
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            ElevatedButton(
              child: const Text('XML Parsiong', style: TextStyle(fontSize: 24)),
              onPressed: () {
                getXMLData();
              },
            ),
...
''';
    ...
    // 확진자 폭증으로 누적검사자수는 현재 집계 x
    print("날짜 - 누적확진자 - 누적사망자");
    // 개선된 for문으로 반복파싱
    for (var item in data2) {
      String createDt = item['createDt'];
      // 파싱된 데이터는 무자열이므로 연산을 위해서는 정수로 변환이 필요함.
